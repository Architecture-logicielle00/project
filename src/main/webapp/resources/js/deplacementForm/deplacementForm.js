...
function supprimerLignesSelectionne(){
        $("#table-depense-deplacement").find('tr > td > input:checked').parent().parent().remove();
        $(".sous-total").change();}
...
function initialiserEvenements(){
    ...
    $('body').on("change", ".row-input", function(){
        ...
        var sousTotalCalcul = $inputsRow.filter(".distance").val() * $inputsRow.filter(".cout-km").val() +
                                $inputsRow.filter(".nb-repas").val() * $inputsRow.filter(".cout-repas").val() +
